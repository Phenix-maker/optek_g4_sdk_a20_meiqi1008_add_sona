...
void audio_arc_input_rx_start(U32 *p_data, U32 size);
static void audio_arc_input_rx_cb(void);
...
void audio_arc_input_rx_init(void)
{
	DBG_Printf("%s\n\r", __func__);
    ...
    memset(&audio_arc_input_env, 0, sizeof(audio_arc_input_env));
  
    #ifdef AUDIO_INPUT_RX_DMA_CLOSE
    if (arcRx_Dma_Opened == FALSE)
    {
        DBG_Printf("arc Rx Dma Open\n\r");
        arcRx_Dma_Opened = TRUE;

        hal_dma_transfer_init(ARC_IN_RX_DMA_ID, NULL, (U32 *)(DA_SPDIF_IN_FIFO_ADDR), SOURCE_DMA_SPDIF_IN,
                        DMA_TRANS_4BYTEs, DMA_BURST_FOUR, 0, 1, 1, audio_arc_input_rx_cb);

        DMA_GCNTR = (0x00001000 | (DMA_CHANNEL(ARC_IN_RX_DMA_ID)));
    }
    else
    {
        DBG_Printf("arc Rx Dma Opened\n\r");
    }
    #else
   	DBG_Printf("arc Rx Dma Open\n\r");
    hal_dma_transfer_init(ARC_IN_RX_DMA_ID, NULL, (U32 *)(DA_SPDIF_IN_FIFO_ADDR), SOURCE_DMA_SPDIF_IN, 
    ...
    audio_arc_input_rx_start(pBuf, AUDIO_INPUT_BUF_SIZE/4);

#else

    memset(&audio_arc_input_env, 0, sizeof(audio_arc_input_env));

    hal_dma_transfer_init(ARC_IN_DMA_ID, NULL, (U32 *)(DA_SPDIF_IN_FIFO_ADDR), SOURCE_DMA_SPDIF_IN, 
                            DMA_TRANS_4BYTEs, DMA_BURST_FOUR, 0, 1, 1, audio_arc_input_rx_cb);

    DMA_GCNTR = (0x00001000 | (DMA_CHANNEL(ARC_IN_DMA_ID)));
...
void audio_arc_input_rx_start(U32 *p_data, U32 size)
{
    ...
    hal_dma_receive_data(ARC_IN_RX_DMA_ID, p_data, size);
    hw_audio_mi2s_rx_start();
#else
    DBG_Assert(p_data != NULL);
    
    hal_dma_receive_data(ARC_IN_DMA_ID, p_data, size);
    audio_arc_input_env.size = 0;
#endif
}

void audio_arc_input_rx_stop_req(void)
{
    DBG_Printf("%s\n\r", __func__);

    audio_arc_input_env.receive_stop_req = TRUE;
    audio_arc_input_rx_disable();
}

U32 audio_arc_input_ready_to_rx_dataPtr(void)
{
    return (audio_arc_input_env.size == 0);
}

static void audio_arc_input_rx_cb(void) __INTERNAL_RAM_TEXT;
static void audio_arc_input_rx_cb(void)
{
#ifdef AUDIO_IN_STREAM_USED
    int rx_length;
    ...
    if (sync_w_src_control.sync_clks.src_type == ASOURCE_SPDIF)
    {
    ...
    #ifdef SOFT_INT_6_USED
    qMsgData->source = AUDIO_INPUT_RX_SOURCE_ARC;
    ...
    if (audio_arc_input_env.receive_enable == FALSE)
    {
        return;
    }
    ...
    if (audio_arc_input_env.receive_stop_req)
    {
        audio_arc_input_env.receive_stop = TRUE;
        audio_arc_input_env.receive_stop_req = FALSE;//clear request
        return;
    }

    if (audio_arc_input_env.size)
    {
        hal_dma_receive_data(ARC_IN_DMA_ID, audio_arc_input_env.p_data, audio_arc_input_env.size);
        audio_arc_input_env.size = 0;
    }
    else
    {
        hal_dma_receive_data(ARC_IN_DMA_ID, audio_arc_input_env.dummy_buf, sizeof(audio_arc_input_env.dummy_buf)/4);
        audio_arc_input_env.overflow_flag = 1;
        DBG_Assert(FALSE);
    }
...
{
    DBG_Printf("%s\r\n", __func__);
    audio_arc_input_env.receive_enable =  TRUE;
}

void audio_arc_input_rx_disable(void)
{
    DBG_Printf("%s\r\n", __func__);
    audio_arc_input_env.receive_enable =  FALSE;

    #ifdef AUDIO_INPUT_RX_DMA_CLOSE
	arcRx_Dma_Opened = FALSE;
    hal_dma_close(ARC_IN_RX_DMA_ID);
    #endif
}

void audio_arc_input_rx_disable_from_isr(void)
{
    audio_arc_input_env.receive_enable =  FALSE;
}
