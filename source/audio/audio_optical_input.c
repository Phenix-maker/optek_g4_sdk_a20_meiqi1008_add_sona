...
U8 spdifRX_Dma_Opened;
#endif


void audio_optical_input_rx_start(U32 *p_data, U32 size);
static void audio_optical_input_rx_cb(void);
...
void audio_optical_input_rx_init(void)
{
	DBG_Printf("%s\n\r", __func__);
    ...
    memset(&audio_optical_input_env, 0, sizeof(audio_optical_input_env));
  
    #ifdef AUDIO_INPUT_RX_DMA_CLOSE
    if (spdifRX_Dma_Opened == FALSE)
    {
    	DBG_Printf("spdif Rx Dma Open\n\r");
        spdifRX_Dma_Opened = TRUE;
    ...
    audio_optical_input_rx_start(pBuf, AUDIO_INPUT_BUF_SIZE/4);
...
void audio_optical_input_rx_stop_req(void)
{
    DBG_Printf("%s\n\r", __func__);

    audio_optical_input_env.receive_stop_req = TRUE;
    audio_optical_input_rx_disable();
}

U32 audio_optical_input_ready_to_rx_dataPtr(void)
{
    return (audio_optical_input_env.size == 0);
}

static void audio_optical_input_rx_cb(void) __INTERNAL_RAM_TEXT;
static void audio_optical_input_rx_cb(void)
{
#ifdef AUDIO_IN_STREAM_USED
    int rx_length;
    ...
    #ifdef SOFT_INT_6_USED
    qMsgData->source = AUDIO_INPUT_RX_SOURCE_OPTICAL;
    ...
    if (audio_optical_input_env.receive_enable == FALSE)
    {
        return;
    }
    ...
    if (audio_optical_input_env.receive_stop_req)
    {
        audio_optical_input_env.receive_stop = TRUE;
        audio_optical_input_env.receive_stop_req = FALSE;//clear request
        return;
    }

    if (audio_optical_input_env.size)
    {
        hal_dma_receive_data(SPDIF_IN_RX_DMA_ID, audio_optical_input_env.p_data, audio_optical_input_env.size);
        audio_optical_input_env.size = 0;
    }
    else
    {
        hal_dma_receive_data(SPDIF_IN_RX_DMA_ID, audio_optical_input_env.dummy_buf, sizeof(audio_optical_input_env.dummy_buf)/4);
        audio_optical_input_env.overflow_flag = 1;
        DBG_Assert(FALSE);
    }
...
{
    DBG_Printf("%s\r\n", __func__);
    audio_optical_input_env.receive_enable =  TRUE;
}

void audio_optical_input_rx_disable(void)
{
    DBG_Printf("%s\r\n", __func__);
    audio_optical_input_env.receive_enable =  FALSE;
...
void audio_optical_input_rx_disable_from_isr(void)
{
    audio_optical_input_env.receive_enable =  FALSE;
}
...
void SoftInt6Isr (void)
{
    ...
    DBG_Assert(qMsgData == &audio_input_rx_task_msg);
