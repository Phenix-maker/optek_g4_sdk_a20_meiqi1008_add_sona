...
void optek_link_codec_init(U16 sample_rate,U8 bt_src)
{
	...
	if (global_vari.optek_link_role == OPL_ROLE_SLAVE)
	{
		...
		{
			if (xa_lc3_enc_main_init(115*8*200, sample_rate, 2, 5.0) != XA_NO_ERROR)
			{
    ...
    {
        if (xa_lc3_dec_handle_init(&opl_envs.decodeHandle, 115*8*200, audio_in_stream.sample_rate, 2, 5.0) != XA_NO_ERROR)
        {
