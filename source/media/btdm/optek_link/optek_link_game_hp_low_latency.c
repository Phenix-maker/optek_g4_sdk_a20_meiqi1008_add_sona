...
#ifdef OPL_MODE_GAME_HP_LOW_LATENCY
...
void optek_link_codec_init(U16 sample_rate,U8 bt_src)
{
	...
	if (global_vari.optek_link_role == OPL_ROLE_SLAVE)
	{
		...
		{
			if (xa_lc3_enc_main_init(55*400*8, sample_rate, 2, 2.5) != XA_NO_ERROR)
			{
    ...
    {
        if (xa_lc3_dec_handle_init(&opl_envs.decodeHandle, 55*400*8, audio_in_stream.sample_rate, 2, 2.5) != XA_NO_ERROR)
        {
...
{
	*dma_end_time = dma_end_bt_time;//rwip_time_get();
