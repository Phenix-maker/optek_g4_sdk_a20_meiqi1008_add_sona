...
U8 Audio_MA12070_read_reg_R(U8 i2c_addr, U8 reg_addr)
{
    DBG_Printf("%s\n\r",__func__);
...
U8 Audio_MA12070P_gpio_init_R(void)
{
    DBG_Printf("%s\n\r",__func__);
    #ifdef AUDIO_MA12070P_FUNCTION_ENABLE
    app_i2c1_init();
...
U8 Audio_MA12070P_gpio_init_end_R(void)
{
    DBG_Printf("%s\n\r",__func__);
    #ifdef AUDIO_MA12070P_FUNCTION_ENABLE
...
void Audio_Dev_MA12070P_Init_R(void)
{
    ...
    #endif
    I2C1_write_sub(DEV_PHY_ADDR_R,0x35, &abData, sizeof(abData));// i2s format  is i2s, Right justified 16bits
    ...
    #endif
    I2C1_write_sub(DEV_PHY_ADDR_R,0x1D, &abData, sizeof(abData));// Power Mode Profile 1
    ...
    #endif
    I2C1_write_sub(DEV_PHY_ADDR_R,0x36, &abData, sizeof(abData));//C use right channel,set i2s profile config
    ...
    #endif
    I2C1_write_sub(DEV_PHY_ADDR_R,0x40, &abData, sizeof(abData));//master integer dB volume,[7:0] effective
    ...
    #endif
    I2C1_write_sub(DEV_PHY_ADDR_R,0x41, &abData, sizeof(abData));//master fract  dB volume,[1:0] effective
...
void Audio_Dev_MA12070P_Open_R(void)
{
    DBG_Printf("%s\n\r",__func__);
    #ifdef AUDIO_MA12070P_FUNCTION_ENABLE
    //Audio_Dev_MA12070P_Set_Mute(TRUE);
    #endif
}
...
void Audio_Dev_MA12070P_Set_Volume_R(U8 volume)
{
    ...
    I2C1_write_sub(DEV_PHY_ADDR_R,0x40, &integer_db, sizeof(integer_db));
    Audio_MA12070_read_reg(DEV_PHY_ADDR_R,0x40);

    I2C1_write_sub(DEV_PHY_ADDR_R,0x41, &fractional_db, sizeof(fractional_db));
    Audio_MA12070_read_reg(DEV_PHY_ADDR_R,0x41);
...
void Audio_Dev_MA12070P_Set_Mute_R(U8 con)
{
    ...
    if (con == TRUE)
    {
        ...
        p_0x36_data.audio_proc_mute = TRUE;             
        #endif
        I2C1_write_sub(DEV_PHY_ADDR_R,0x36, &abData, sizeof(abData));
        Audio_MA12070_read_reg(DEV_PHY_ADDR_R,0x36);
    }
    else if (con == FALSE)
    {
        ...
        #endif
        I2C1_write_sub(DEV_PHY_ADDR_R,0x36, &abData, sizeof(abData));
        Audio_MA12070_read_reg_R(DEV_PHY_ADDR_R,0x36);
    }
    else
    {
